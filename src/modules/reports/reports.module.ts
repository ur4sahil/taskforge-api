...
@Injectable() export class ReportsService {
  ...
  async personal(wid: string, mid: string) {
    ...
    const [open, overdue, dueToday, done7d] = await Promise.all([
      ...
      this.prisma.task.count({ where: { workspaceId: wid, assigneeId: mid, dueDate: { gte: today, lt: new Date(today.getTime()+86400000) }, deletedAt: null } }),
      this.prisma.task.count({ where: { workspaceId: wid, assigneeId: mid, status: 'done', completedAt: { gte: new Date(Date.now()-7*86400000) }, deletedAt: null } })
    ]);
  ...
  async team(wid: string, managerId: string) {
    ...
    const stats = await Promise.all(reports.map(async r => ({ name: r.user.name, ...await this.personal(wid, r.id) })));
...
@Controller('workspaces/:wid/reports') @UseGuards(WorkspaceGuard) export class ReportsController {
  constructor(private svc: ReportsService) {}
  @Get('my-stats') async my(@Param('wid') w: string, @CurrentMember() m: CurrentWorkspaceMember) { return successResponse(await this.svc.personal(w, m.id)); }
  @Get('team') @UseGuards(RolesGuard) @Roles('manager') async team(@Param('wid') w: string, @CurrentMember() m: CurrentWorkspaceMember) { return successResponse(await this.svc.team(w, m.id)); }
...
@Module({ controllers: [ReportsController], providers: [ReportsService], exports: [ReportsService] }) export class ReportsModule {}
