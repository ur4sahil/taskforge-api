...
export class EmailIngestionService {
  ...
  async process(payload: any) {
    ...
    const list = await this.prisma.list.findFirst({ where: { inboundEmail: to, inboundEmailEnabled: true, archivedAt: null } });
    if (!list) throw new BadRequestException('Unknown address');
    const task = await this.prisma.task.create({
      data: { workspaceId: list.workspaceId, listId: list.id, title: (payload.subject || 'Untitled').substring(0, 255), description: payload.text || '', status: 'todo' as any, priority: 'medium' as any, creatorId: list.createdById, assigneeId: list.defaultAssigneeId, source: 'email' as any },
    });
    ...
    this.logger.log(`Email task ${task.id} created`);
    return task;
  }
}
...
export class EmailIngestionController {
  constructor(private svc: EmailIngestionService) {}
  @Public() @Post('inbound-email') async handle(@Body() p: any) { return successResponse({ taskId: (await this.svc.process(p)).id }); }
}
